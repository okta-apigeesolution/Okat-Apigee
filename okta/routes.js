...
module.exports = function (app) {
	...
	app.post('/getAccessToken', function(req, res, next) {
	   ...
	   var customScope = "";
		...
		console.log("the flow code is: " + flow);
		...
		if(flow == "clientCred"){
			...
			customScope = "http://myapp.com/scp/gold";
			var options = {
			method: 'POST',
			url: process.env.ISSUER + "/v1/token",
			qs: {
...
if(flow == "authCode"){
	...
	console.log("response"+JSON.stringify(response_to_browser))
	res.json(JSON.stringify(response_to_browser))
}
		})
	})
	...
	app.post('/getRefreshToken', function(req, res, next) {
		var code = req.body.code;
...
console.log(obj);
			...
			response_to_browser.access_token = obj.access_token
			response_to_browser.id_token = obj.id_token
			response_to_browser.refresh_token = obj.refresh_token
